 ...
 */
 ...
 int parse_command_sequence(char *input, command_list_t *clist);

 int parse_command(char *input, command_list_t *clist) {
    return parse_command_sequence(input, clist);
}
 ...
 */
 int exec_client_requests(int cli_socket) {
    ...
    while (1) {
        ...
        if (parse_command(io_buff, &cmd_list) != 0) {
    ...
    continue;
        ...
        if (cmd_rc != 0) {
            send_message_string(cli_socket, "Command failed\n");
        }
        send_message_eof(cli_socket);
...
int send_message_string(int cli_socket, char *buff){
    ...
    sent_len = send(cli_socket, buff, buff_len, 0);
    if (sent_len < 0) {
        perror("send");
        return ERR_RDSH_COMMUNICATION;
    }
