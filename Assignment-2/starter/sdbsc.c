...
int get_student(int fd, int id, student_t *s)
{
    ...
    if (memcmp(s, &EMPTY_STUDENT_RECORD, sizeof(student_t)) == 0 || s->id != id)
...
int add_student(int fd, int id, char *fname, char *lname, int gpa)
{
 student_t new_student = {0};
    ...
    if (read(fd, &existing_student, sizeof(student_t)) != sizeof(student_t)) {
        memset(&existing_student, 0, sizeof(student_t));
    }
    if (memcmp(&existing_student, &EMPTY_STUDENT_RECORD, sizeof(student_t)) != 0) {
...
int del_student(int fd, int id)
{
    ...
    if (read(fd, &student_to_delete, sizeof(student_t)) != sizeof(student_t)) {
        printf(M_ERR_DB_READ);
        return ERR_DB_FILE;
    }
    if (memcmp(&student_to_delete, &EMPTY_STUDENT_RECORD, sizeof(student_t)) == 0) {
...
int main(int argc, char *argv[])
{
	...
	if (ftruncate(fd, MAX_STD_ID * STUDENT_RECORD_SIZE) == -1) {
            perror("ftruncate");
            exit_code = EXIT_FAIL_DB;
            break;
        }
